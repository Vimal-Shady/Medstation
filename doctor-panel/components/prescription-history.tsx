...
export default function PrescriptionHistory({ prescriptions, isLoading, onRefresh }: PrescriptionHistoryProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: data.details || "Prescription and payment status have been updated",
        })
        onRefresh()
      } else {
  ...
  const filteredPrescriptions = prescriptions.filter((prescription) => {
    ...
    // Filter by search term (patient name or medicine name)
    if (
      searchTerm &&
      !prescription.patient_name?.toLowerCase().includes(searchTerm.toLowerCase()) &&
      !prescription.medicines?.toLowerCase().includes(searchTerm.toLowerCase())
    ) {
      return false
    }
                        ...
                        {isLoadingDetails ? (
                          ...
                          <div className="space-y-4">
                            <div>
                              ...
                              <p>{prescriptionDetails.patient_name}</p>
                            </div>
